...
namespace ANS::Test
{
	...
	public:
		...
		Compression(String location, String testName) : testName(testName)
		{
			...
			this->metan	 = location + COMPRESSED_OUT_DIR + testName + ".vmeta";
		...
		int run(bool generate = false)
		{
			...
			auto testOut = [&]() {
				...
				while(!encout.empty())
				{
					...
					if(this->isVerbose)
					{
						...
						if((i + 1) % (sizeof(state_t) / sizeof(message_t) * 4)
						   == 0)
						{ std::cout << "\n         "; }
						i++;
					}
				...
				while(!encmeta.empty())
				{
					...
					if(this->isVerbose)
					{
						...
						std::cout
							...
							<< "\tFin Channel: " << (int) cur.current_channel
							<< "\n"
			...
			ANS::compress(message, encout, encmeta, padding, control);
	...
	public:
		...
		Decompression(String location, String testName) : testName(testName)
		{
			...
			this->inmn	= location + COMPRESSED_OUT_DIR + testName + ".vmeta";
			this->goutn = location + IN_DIR + testName + ".vin";
			this->outn	= location + DECOMPRESSED_OUT_DIR + testName + ".vout";
			this->isVerbose = false;
		}
