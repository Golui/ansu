									  ...
									  backend::stream<message_t>& message)
{
	...
	//		cur_channel			= (CHANNEL_COUNT + cur_channel - 1) %
	//CHANNEL_COUNT;
	//	}
	//
	...
	//		reverse_msg.pop();
	//
	//	// Same here; since we popped CHANNEL_COUNT elements off at the
	//beginning 	master.messagePad -= CHANNEL_COUNT; 	while(!reverse_msg.empty()
	//&& reverse_msg.size() > (master.messagePad))
	//	{
